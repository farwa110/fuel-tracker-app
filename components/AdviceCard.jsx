...
// import { Lightbulb, CircleDollarSign, MapPin, TriangleAlert, CircleCheck, ChartNoAxesCombined, Calculator, Fuel, Money } from "lucide-react";
...
import { motion, AnimatePresence } from "framer-motion";
import { Lightbulb, CircleDollarSign, MapPin, TriangleAlert, CircleCheck, ChartNoAxesCombined, Calculator, Fuel } from "lucide-react";

function generateAdvice({ allStations, fuel, hasLocation, stations }) {
  const tips = [];

  if (!hasLocation) return tips;
  if (!allStations.length) return tips;
...
export default function AdviceCard({ allStations = [], stations = [], fuel = "benzin95", hasLocation = false }) {
  ...
  return (
    ...
    >
      ...
      >
        <Lightbulb size={15} strokeWidth={2.2} style={{ color: "var(--accent)" }} />
